...
async function getproductosxcategoria(idCategoria) {
  const options = {
    method: "GET",
    url: "http://192.168.0.7:4000/api/buscaproductosxcategoria/"+idCategoria,
    headers: { "Content-Type": "application/json", "xx-token": token },
  };
  await axios(options)
    .then((response) => {
  ...
  return (
    <View style={styles.containerroot}>
      <View style={styles.containerchild}>
         <Text style={styles.titulo1}>Bienvenido: {user.nombre+" "+user.apellido}</Text>
         ...
         <Icon name="home" size={30} color="black"style={{height:"auto"}}  />
         </View>
      ...
      <View style={styles.containerdir}>
          <IconExpo  name='place' size={30}  color="black"style={{height:"auto"}}  />
        ...
        <ScrollView showsHorizontalScrollIndicator={false} horizontal={true} >
      {
        categorias.map((categoria)=>{
           return <TouchableOpacity onPress={()=>{console.log('object :>> ', categoria.id);getproductosxcategoria(categoria.id)}} style={styles.btn} key={categoria.id}>
     ...
     <TouchableOpacity onPress={()=>{gettodosproductos()}}> 
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return <TouchableOpacity
              onPress={()=>{console.log('object :>> ', item.id)}}
            
            ><View style={styles.cardproducto} key={item.id}>
            <Image
              source={{ uri: "http://192.168.0.7:4000/" + item.imagen }}
              style={{
                width: 150,
                height: 100,
            ...
            >
              ...
              <Text style={styles.textsignup1}>{"$" + item.precio}</Text>
            </View>
